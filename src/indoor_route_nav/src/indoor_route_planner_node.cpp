...
#include "indoor_route_nav/srv/plan_path.hpp"
...
std::vector<Polygon> parseAreasJson(const std::string & areas_json)
{
  std::vector<Polygon> polys;
  const auto root = nlohmann::json::parse(areas_json);
  if (!root.is_array()) {
    throw std::runtime_error("可行驶区域格式错误");
  }

  for (const auto & area : root) {
    const auto points_it = area.find("points");
    if (points_it == area.end() || !points_it->is_array()) {
      continue;
    }

    Polygon poly;
    for (const auto & item : *points_it) {
      if (!item.is_object() || !item.contains("x") || !item.contains("y")) {
        continue;
      }
      Point p;
      p.x = item.at("x").get<double>();
      p.y = item.at("y").get<double>();
      p.z = item.value("z", 0.0);
      poly.push_back(p);
    }
    if (poly.size() >= 3) {
...
std::vector<Point> planPathInDrivableAreas(
  const Point & start,
  const Point & goal,
...
geometry_msgs::msg::PoseStamped makePose(
  const Point & p,
  const geometry_msgs::msg::PoseStamped & template_pose)
{
  geometry_msgs::msg::PoseStamped pose = template_pose;
...
public:
  PlannerNode()
  : Node("indoor_route_planner_node")
  {
    path_pub_ = create_publisher<nav_msgs::msg::Path>("/indoor_route_nav/planner/path", 10);
    status_pub_ = create_publisher<std_msgs::msg::String>("/indoor_route_nav/planner/status", 10);
    plan_service_ = create_service<indoor_route_nav::srv::PlanPath>(
      "/indoor_route_nav/planner/plan_path",
      std::bind(&PlannerNode::handlePlan, this, std::placeholders::_1, std::placeholders::_2));

    publishStatus("indoor_route_nav C++ planner service ready");
  }

private:
  void handlePlan(
    const std::shared_ptr<indoor_route_nav::srv::PlanPath::Request> request,
    std::shared_ptr<indoor_route_nav::srv::PlanPath::Response> response)
  {
    try {
      const auto polys = parseAreasJson(request->drivable_areas_json);
      const Point start{
      ...
      const auto points = planPathInDrivableAreas(
        start, goal, polys, request->resolution, request->robot_radius);

      nav_msgs::msg::Path path;
      path.header = request->goal.header;
      path.header.stamp = now();
      path.poses.reserve(points.size());
      for (const auto & p : points) {
        auto pose = makePose(p, request->goal);
        pose.header = path.header;
        path.poses.push_back(pose);
      }

      response->success = true;
      response->message = "C++自动规划完成，路径点 " + std::to_string(points.size()) + " 个";
      response->path = path;
      ...
      publishStatus(std::string("C++自动规划失败: ") + e.what());
    }
  }
  ...
  rclcpp::Service<indoor_route_nav::srv::PlanPath>::SharedPtr plan_service_;
};
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<indoor_route_nav::PlannerNode>());
  rclcpp::shutdown();
  return 0;
}
